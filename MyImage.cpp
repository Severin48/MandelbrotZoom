...
void CMyImage::Copy(const CMyImage& toCopy)
{
    ...
    unsigned char* copy_data = toCopy.m_pData;
    unsigned char* end = copy_data + size;
    for (unsigned char* i = m_pData; i <= end; i++) {
        *i = *copy_data;
    }
}
...
void CMyImage::Resize(int width, int height)
{
    ...
    int old_size = m_width * m_height;
    unsigned char* tmp = (unsigned char*)malloc(old_size);;
    if (new_size > 0) {
        if (old_size > 0 && tmp != nullptr) {
            memcpy(tmp, m_pData, old_size);
        }
        free(m_pData);
        m_pData = (unsigned char*) malloc(new_size);
        if (tmp != nullptr && m_pData != nullptr) {
            memcpy(m_pData, tmp, old_size);
            free(tmp);
        }
        m_width = width;
        m_height = height;
